...
alter table public.evaluation_questions drop constraint if exists evaluation_questions_audience_check;
alter table public.evaluation_questions add constraint evaluation_questions_audience_check
  check (audience in ('all', 'internal', 'exchange', 'leadership', 'affiliate_peer', 'leader_peer'));

create or replace function public.evaluation_question_audience(
  p_evaluator_type text, p_target_type text, p_evaluator_company text, p_target_company text
) returns text language sql immutable set search_path = public, pg_temp as $$
  select case
    when trim(coalesce(p_evaluator_type, '')) in ('팀장/부서장급', '팀장급', '부서실장급')
     and trim(coalesce(p_target_type, '')) in ('팀장/부서장급', '팀장급', '부서실장급') then 'leader_peer'
    when trim(coalesce(p_evaluator_type, '')) = '팀원급'
     and trim(coalesce(p_target_type, '')) = '팀원급'
     and trim(coalesce(p_evaluator_company, '')) <> ''
     and trim(coalesce(p_target_company, '')) <> ''
     and trim(p_evaluator_company) <> trim(p_target_company) then 'affiliate_peer'
    else 'all'
  end
$$;
...
as $$
  with assignment as (
    select m.cycle_id,
      ...
      public.evaluation_question_audience(evaluator.type::text, target.type::text, evaluator.company::text, target.company::text) question_audience
    ...
    where m.id = p_matching_id and evaluator.auth_user_id = auth.uid()
      ...
      and current_date between cycle.start_date and cycle.deadline and cycle.status = '진행중'
  )
  select question.id, question.cycle_id, question.category, question.text,
         question.weight, question.type, question.target_track, question.target_dept,
         question.audience, question.required, question.is_default, question.max_score
  ...
  where coalesce(question.audience, 'all') = assignment.question_audience
    and case
      when assignment.question_audience = 'affiliate_peer'
        then public.canonical_question_track_alias(question.target_track) in ('all', 'headquarters_member')
      when assignment.question_audience = 'leader_peer'
        then public.canonical_question_track_alias(question.target_track) in ('all', 'headquarters_leader')
...
as $$
  with assignments as (
    ...
    ) category from assignments assignment
  ), missing as (
    select required.* from required_categories required where not exists (
      select 1 from public.evaluation_questions question
      where question.cycle_id = p_cycle_id and question.category = required.category
        and coalesce(question.type, '') <> '서술형' and coalesce(question.weight, 0) > 0 and coalesce(question.max_score, 0) > 0
        and coalesce(question.audience, 'all') = required.question_audience
        and case
          when required.question_audience = 'affiliate_peer'
            then public.canonical_question_track_alias(question.target_track) in ('all', 'headquarters_member')
          when required.question_audience = 'leader_peer'
            then public.canonical_question_track_alias(question.target_track) in ('all', 'headquarters_leader')
    ...
    select 'QUESTION_COVERAGE_MISSING' code, target_track, category,
      format('%s / %s: %s 질문이 없습니다.', target_track,
        case question_audience when 'affiliate_peer' then '계열사 팀원 간 평가'
          when 'leader_peer' then '팀장·부서장 간 협업 평가' else '일반 평가' end, category) message
    from missing
    union all
    select distinct 'EXECUTIVE_TARGET_ASSIGNED', 'executive', null::text, '임원급 인원은 직접 평가 대상에 포함할 수 없습니다.'
    from public.matchings matching join public.users target on target.id = matching.target_id
    where matching.cycle_id = p_cycle_id and target.type::text in ('임원급', '임원')
  )
  select jsonb_build_object('ok', not exists(select 1 from issues), 'issues', coalesce(jsonb_agg(jsonb_build_object(
    'code', code, 'target_track', target_track, 'category', category, 'message', message)), '[]'::jsonb)) from issues
$$;
...
begin
  ...
  select m.cycle_id, m.evaluator_id, m.target_id,
    ...
    public.evaluation_question_audience(evaluator.type::text, target.type::text, evaluator.company::text, target.company::text)
  ...
  v_categories := case when v_question_audience = 'affiliate_peer' then array['성과','협업','성장','조화']
  ...
  if (select count(*) <> count(distinct answer.question_id) from jsonb_to_recordset(coalesce(p_answers, '[]'::jsonb)) answer(question_id bigint, score numeric))
  ...
  if exists (select 1 from public.my_assigned_questions(p_matching_id) q where coalesce(q.type, '') <> '서술형' and q.required is true
    and not exists (select 1 from jsonb_to_recordset(coalesce(p_answers, '[]'::jsonb)) answer(question_id bigint, score numeric)
...
revoke all on function public.evaluation_question_audience(text,text,text,text) from public, anon;
...
grant execute on function public.evaluation_question_audience(text,text,text,text) to authenticated, service_role;
