...
begin
  ...
  if p_approver_ids is null or cardinality(p_approver_ids) not between 1 and 3
     ...
     or p_actor_id = any(p_approver_ids) then
    raise exception 'One to three unique executive approvers are required';
  end if;
  if exists (
  ...
  if exists (
    select 1 from public.evaluation_cycle_approval_requests
    where cycle_id=p_cycle_id and request_status='requested'
  ) then raise exception 'Approval already requested'; end if;
  ...
  update public.evaluation_cycles set
    internal_approval_status='requested',
