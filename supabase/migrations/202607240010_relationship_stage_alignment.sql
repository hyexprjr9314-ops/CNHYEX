...
-- Keep persisted assignment relationships aligned with the canonical API rule:
...
with relationship_context as (
  select
    matching.id,
    coalesce(evaluator.company::text, '') as evaluator_company,
    coalesce(target.company::text, '') as target_company,
...
set relationship_type = case
  ...
  when context.evaluator_company = context.target_company
    and context.evaluator_dept = context.target_dept then 'internal'
  else 'exchange'
end
