...
with profiles as (
  select
    ...
    replace(
      regexp_replace(
        case when trim(coalesce(workplace, '')) not in ('', '본사') then workplace else dept end,
        '[[:space:]ㆍ·._-]+', '', 'g'
      ),
      '영업소', ''
    ) as branch_key
  ...
  where cycle.status::text in ('초안', 'draft', 'not_started')
    ...
    and not (
      (
        evaluator.is_mechanic and target.is_mechanic
        and trim(evaluator.company::text) = trim(target.company::text)
        and trim(coalesce(evaluator.dept, '')) = trim(coalesce(target.dept, ''))
      )
      or (
        ((evaluator.is_mechanic and target.is_branch) or (evaluator.is_branch and target.is_mechanic))
        and trim(evaluator.company::text) = trim(target.company::text)
        and evaluator.branch_key <> ''
        and evaluator.branch_key = target.branch_key
      )
      or (
        evaluator.is_vehicle_safety and target.is_mechanic
        and trim(evaluator.company::text) = trim(target.company::text)
      )
    )
)
