...
begin
  ...
  set approved_grade = coalesce((
    select a.grade_override
    from public.evaluation_result_adjustments a
    where a.cycle_id = r.cycle_id and a.target_id = r.target_id and a.status = 'active'
  ), r.relative_grade)
  where r.cycle_id = p_cycle_id
    and r.result_version = (
      select c.result_version from public.evaluation_cycles c where c.id = p_cycle_id
    );
