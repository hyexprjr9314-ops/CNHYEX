...
test('mechanic assignment repair rejects every unsupported one-sided draft pairing', async () => {
  const migrations = await Promise.all([
    ...
    '../supabase/migrations/202608080008_align_branch_vehicle_mechanic_precedence.sql'
  ].map(path => readFile(new URL(path, import.meta.url), 'utf8')));
  for (const sql of migrations) {
    assert.match(sql, /evaluator\.is_mechanic or target\.is_mechanic/);
    assert.match(sql, /and not case[\s\S]*when evaluator\.is_mechanic and target\.is_mechanic[\s\S]*when \(evaluator\.is_mechanic and target\.is_branch\)[\s\S]*and evaluator\.is_branch and target\.is_branch[\s\S]*when evaluator\.is_vehicle_safety and target\.is_mechanic[\s\S]*when evaluator\.is_mechanic or target\.is_mechanic then false/);
