...
test('mobile administrator dashboard uses compact cards, short labels, and non-overlapping actions', async () => {
  ...
  assert.match(index, /#view-admin th:last-child \{ font-size: 0 !important; \}[\s\S]*#view-admin th:last-child::after[\s\S]*content: "관리"/);
  assert.match(index, /<span class="md:hidden">평가 관리<\/span>/);
  assert.match(index, /<span class="md:hidden">집계·마감<\/span>/);
  assert.match(index, /mobile-pending-grade-copy \{ font-size: 0 !important; \}[\s\S]*mobile-pending-grade-copy::after[\s\S]*결과 공개 후 최종 등급이 표시됩니다/);
  assert.match(index, /#summary-filter-toolbar > :not\(#summary-company-segments\) \{[\s\S]*display: none !important/);
  assert.match(index, /data-mobile-label="평가자 켜기"/);
  assert.match(index, /data-mobile-label="피평가자 끄기"/);
  assert.match(index, /#permission-table-head-evaluator,[\s\S]*font-size: 0 !important[\s\S]*permission-table-head-evaluator::after[\s\S]*content: "평가자"/);
  assert.match(index, /#history-access-copy::after[\s\S]*content: "임원·관리자 전용"/);
  assert.match(index, /#history-list-title::after[\s\S]*content: "과거 이력"/);
  assert.match(index, /#history-count-label::before[\s\S]*content: "보관 "/);
