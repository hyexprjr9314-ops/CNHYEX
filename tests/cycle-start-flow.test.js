...
test('automatic matching creates an atomic quota draft that remains manually editable', async () => {
  ...
  assert.match(api, /if \(saved\.error\) return saved;/);
  assert.doesNotMatch(api, /schema cache\|could not find/);
  ...
  assert.match(ui, /if \(activate\)[\s\S]*action:\s*'matching_generate'[\s\S]*현재 인원으로 가능한 배정만 구성했습니다/);
  assert.doesNotMatch(ui, /평가 배정 후보가 부족하여 시작을 중단했습니다/);
