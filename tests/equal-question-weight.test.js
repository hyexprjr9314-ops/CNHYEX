...
test('question weight is server-owned and hidden from all management inputs', async () => {
  const [index, questionTemplate, adminState, questions, migration] = await Promise.all([
    read('../index.html'),
    read('../evaluation-question-template.csv'),
  ...
  assert.match(questionTemplate, /성과,영업소 직원,5지선다형/);
