...
const autoMatching = new URL('../api/auto-matching.js', import.meta.url);
...
test('manual matching remains unrestricted while automatic matching keeps policy boundaries', async () => {
  const [apiSource, autoSource, indexSource] = await Promise.all([
    readFile(api, 'utf8'),
    readFile(autoMatching, 'utf8'),
    readFile(index, 'utf8')
  ]);
  assert.match(autoSource, /allowedMatchingPair/);
  assert.doesNotMatch(apiSource, /정비사 또는 영업소 매칭 조건에 맞지 않는 대상입니다/);
  assert.doesNotMatch(indexSource, /allowedInteractiveMatchingPair/);
  assert.match(indexSource, /target\.id !== selectedEvaluatorForMatching[\s\S]*target\.is_evaluatee !== false\);/);
